...
DEFAULT_WEBHOOK_URL = "https://hook.eu2.make.com/shrv6ysxc68lb282yxk4idx579j3cmtm"
...
def send_to_make(webhook_url: str, payload: Dict[str, Any] | List[Dict[str, Any]]) -> requests.Response:
...
st.set_page_config(page_title="Excel -> Make webhook", page_icon="📤", layout="wide")
st.title("Excel -> Make webhook")
st.write("Nahrajte Excel, zkontrolujte data a odešlete je jako JSON na webhook v Make.")

with st.sidebar:
    st.header("Nastavení")
    webhook_url = st.text_input("Make webhook URL", value=DEFAULT_WEBHOOK_URL, type="password")
    include_metadata = st.checkbox("Zabalit data do objektu s metadaty", value=True)
    send_only_previewed_rows = st.checkbox("Odeslat pouze prvních N řádků", value=False)
    preview_limit = st.number_input("Počet řádků pro náhled/test", min_value=1, max_value=10_000, value=20, step=1)

uploaded_file = st.file_uploader("Vyberte Excel soubor", type=["xlsx", "xls"])
...
selected_sheet = st.selectbox("List v Excelu", sheet_names)
...
with col1:
    send_clicked = st.button("Odeslat na Make", type="primary")

if send_clicked:
    if not webhook_url.strip():
        st.error("Doplňte URL webhooku.")
    elif not records:
        st.warning("Vybraný list neobsahuje žádné řádky k odeslání.")
    else:
        try:
            response = send_to_make(webhook_url.strip(), payload)
